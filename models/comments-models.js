...
exports.selectCommentsByArticleId = (id, limit = 10, p = 1) => {
    ...
    if (typeof parseInt(limit) !== 'number' || typeof parseInt(p) !== 'number') {
        return Promise.reject({ status: 400, msg: 'Invalid query' })
    }

    return db.query(`SELECT comment_id, votes, created_at, author, body FROM comments WHERE article_id = $1 LIMIT ${limit} OFFSET (${p} - 1) * ${limit};`, [id]).then(({ rows }) => {
        if (rows.length === 0) {
